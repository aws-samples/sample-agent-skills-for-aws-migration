---
name: azure-to-aws
description: "Migrate workloads from Microsoft Azure to AWS. Triggers on: migrate from Azure, Azure to AWS, move off Azure, migrate Azure to AWS, migrate Azure SQL to RDS, migrate AKS to EKS, migrate Azure Functions to Lambda, migrate Container Apps to Fargate, Azure migration. Runs a 6-phase process: discover Azure resources from Terraform files, app code, or billing exports, clarify migration requirements, design AWS architecture, estimate costs, generate migration artifacts, and collect optional feedback. Clarify must finish before Design, Estimate, or Generate. Includes AI provider migration guidance (for example, Azure OpenAI to Amazon Bedrock) by selecting closest-fit Bedrock model families for required modality, latency/quality targets, context windows, and cost constraints. Model mapping is compatibility-guided, not 1:1 parity; validate prompts, tool-calling behavior, and eval metrics before cutover. Do not use for: GCP or on-premises migrations to AWS, AWS-to-Azure reverse migration, general AWS architecture advice without migration intent, Azure-to-Azure refactoring, or multi-cloud deployments that do not involve migrating off Azure."
---
...
- **Re-platform by default**: Select AWS services that match Azure workload types (e.g., Container Apps → Fargate, Azure SQL → RDS, Azure Functions → Lambda).
...
- **No human one-time migration costs**: Do not present human labor, professional services, or people-time work as dollar estimates or "one-time migration cost" budget categories. Vendor charges grounded in data (for example Azure data transfer egress in the infra estimate when billing exists) are allowed.
...
- **Azure Synapse Analytics / `azurerm_synapse_*`**: The skill **does not** recommend a specific AWS analytics or warehouse service. During **Clarify**, if discovery shows Synapse Analytics (IaC `azurerm_synapse_*` and/or billing rows for Azure Synapse), you **must** surface the specialist advisory **before** Design (see `references/phases/clarify/clarify.md`). Design output uses **`Deferred — specialist engagement`**; keep directing the user to their **AWS account team** and/or a **data analytics migration partner** through Design, Estimate, and docs (see `references/phases/design/design-infra.md` Synapse specialist gate).
- **Azure AD / Entra ID**: Do not migrate Azure AD/Entra ID identities to AWS Cognito or any specific AWS identity service. Surface this as a specialist advisory in Clarify; Design marks identity as **`Deferred — identity specialist engagement`**.
...
| File | Condition |
|------|-----------|
| `design-refs/ai-openai-to-bedrock.md` | `ai-workload-profile.json` exists AND `summary.ai_source` = `"azure_openai"` or `"openai"` or `"both"` |
...
User must provide at least one Azure source:

- **Terraform IaC**: `.tf` files (with optional `.tfvars`, `.tfstate`) using `azurerm` provider
- **ARM / Bicep templates**: `azuredeploy.json`, `*.bicep` files
- **Application code**: Source files with Azure SDK or AI framework imports
- **Billing data**: Azure Cost Management export files (CSV or JSON)
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| **Discover** | `.tf` files / ARM / Bicep, app source code, and/or billing exports (at least one required)                                                                               | `azure-resource-inventory.json`, `azure-resource-clusters.json`, `ai-workload-profile.json`, `billing-profile.json`, `.phase-status.json` updated (outputs vary by input)                                                                           | `references/phases/discover/discover.md` |
| **Clarify**  | Discovery artifacts (`azure-resource-inventory.json`, `azure-resource-clusters.json`, `ai-workload-profile.json`, `billing-profile.json` — whichever exist)              | `preferences.json`, `.phase-status.json` updated                                                                                                                                                                                                    | `references/phases/clarify/clarify.md`   |
...
│   │   │   ├── discover-iac.md                 # Terraform/ARM/Bicep IaC discovery
...
│   │
│   ├── design-refs/
│   │   ├── index.md                            # Lookup table: Azure type → design-ref file
│   │   ├── fast-path.md                        # Deterministic 1:1 mappings (Pass 1)
│   │   ├── compute.md                          # Compute mappings (Container Apps, VMs, AKS, etc.)
│   │   ├── database.md                         # Database mappings (Azure SQL, Cosmos DB, etc.)
│   │   ├── storage.md                          # Storage mappings (Blob, Files, etc.)
│   │   ├── networking.md                       # Networking mappings (VNet, NSG, DNS, etc.)
│   │   ├── messaging.md                        # Messaging mappings (Service Bus, Event Hub, etc.)
│   │   ├── security.md                         # Security mappings (Key Vault, Managed Identity, etc.)
│   │   ├── ai-openai-to-bedrock.md             # Azure OpenAI → Bedrock model mapping
│   │   └── ai.md                               # Other AI/ML mappings (Azure ML → SageMaker)
│   │
...
│       ├── schema-discover-iac.md              # azure-resource-inventory + clusters schemas
...
│       ├── pricing-cache.md                    # Cached AWS + Azure pricing (±5-25%, primary source)
│       ├── bedrock-quotas.md                   # Bedrock TPM/RPM quota awareness
│       ├── terraform-validation.md             # Terraform validation gate
│       ├── retarget-gotchas.md                 # Agentic retarget migration gotchas
│       └── ai-migration-guardrails.md          # AI migration safety guardrails
```

| Condition                                                       | Action                                                                                                                                                  |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No Azure sources found (no `.tf`, no ARM/Bicep, no billing data)| Stop. Output: "No Azure sources detected. Provide at least one source type (Terraform files, ARM/Bicep templates, application code, or billing exports) and try again." |
...
- **Region**: `us-east-1` (unless user specifies, or Azure region → AWS region mapping suggests otherwise)
...
- Terraform infrastructure discovery (`azurerm` provider)
- ARM / Bicep template discovery (lightweight scan)
- App code scanning (AI workload detection)
- Billing data import from Azure Cost Management exports
