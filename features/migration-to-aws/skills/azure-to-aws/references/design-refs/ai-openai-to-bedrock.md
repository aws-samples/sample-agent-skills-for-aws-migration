# Azure OpenAI / OpenAI to Bedrock — Model Selection Guide

**Applies to:** OpenAI SDK usage OR Azure OpenAI SDK detected in Azure-hosted applications → Amazon Bedrock

This file is loaded by `design-ai.md` when `ai-workload-profile.json` has `summary.ai_source` = `"azure_openai"`, `"openai"`, or `"both"`. It provides model mapping tables with pricing and honest competitive analysis for Azure OpenAI / OpenAI → Bedrock migration decisions.

Many Azure-hosted applications use either the Azure OpenAI SDK (wrapping OpenAI models deployed in Azure) or the OpenAI API directly. This guide covers both migration paths.
...
## Azure OpenAI SDK → Bedrock

Applications using `azure.ai.openai.AzureOpenAI` or `@azure/openai` (TypeScript) have two migration paths:

### Path A: Mantle (OpenAI-compatible endpoints, minimal code changes)
1. Replace `AzureOpenAI(azure_endpoint=..., api_key=..., azure_deployment=...)` with:
   `OpenAI(base_url="https://bedrock-mantle.{region}.api.aws/v1", api_key=<bedrock-key>)`
2. Change model string to Bedrock model ID
3. Estimated effort: hours to 1 day

### Path B: AWS SDK (boto3 Converse API)
1. Replace `client.chat.completions.create(model="gpt-4o", ...)` with `bedrock.converse(modelId="...", messages=[...])`
2. Map Azure message format to Bedrock format
3. Estimated effort: 1-3 days per integration

> **Azure deployment → model mapping:** Azure OpenAI deployments map to the underlying OpenAI model they serve (e.g., a deployment named `gpt4o-prod` using model `gpt-4o` → select the GPT-4o row in the table below). Identify the base model from the Azure Portal deployment settings or `azurerm_cognitive_deployment` Terraform resource before selecting a Bedrock target.
...
| Azure OpenAI SDK     | Mantle OpenAI-compat endpoints (Path A) or boto3 Converse (Path B) | See Azure OpenAI SDK section above                    |
