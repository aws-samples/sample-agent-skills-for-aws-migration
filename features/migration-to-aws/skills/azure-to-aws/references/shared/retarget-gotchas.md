...
These are real issues startups hit when keeping their existing framework and swapping the model layer to Bedrock. A simple "swap `AzureChatOpenAI` for `ChatBedrock`" understates the work. This file documents what actually breaks and how to fix it.

---

## Azure OpenAI SDK

### `azure_deployment` parameter vs model ID

**Problem:** Azure OpenAI SDK requires both `azure_deployment` (the deployment name in Azure Portal) and optionally `model_name` (the underlying model). When migrating to Bedrock, you only need `model_id` (the Bedrock model ID). Code that relies on `azure_deployment` as the key identifier will need refactoring — there is no Azure deployment concept in Bedrock.

**Fix:** Replace `azure_deployment` references with the Bedrock `model_id` directly. Update any configuration or environment variables that store deployment names to store Bedrock model IDs instead (e.g., `"gpt-4o"` Azure deployment → `"anthropic.claude-sonnet-4-6"` Bedrock model ID).

### `AzureOpenAI` client configuration

**Problem:** `AzureOpenAI(azure_endpoint=..., api_key=..., api_version=...)` requires Azure-specific parameters. Bedrock uses boto3 with IAM credentials — no API key, no endpoint URL, no API version parameter.

**Fix:** Replace the `AzureOpenAI` client with boto3 Bedrock Runtime client or the `ChatBedrock` LangChain class. Remove `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_ENDPOINT`, and `AZURE_OPENAI_API_VERSION` env vars. Add AWS credentials configuration.

### Semantic Kernel Azure plugins

**Problem:** Semantic Kernel has Azure-specific kernel plugins (AzureChatCompletion, AzureTextEmbedding) and Azure-specific prompt template syntax. These are not directly portable to Bedrock.

**Fix:** Replace Azure kernel plugins with LangChain-based equivalents or direct boto3 calls. Semantic Kernel's `AzureChatCompletion` → `ChatBedrock` (LangChain) or boto3 Bedrock Runtime `invoke_model`. Kernel memory with Azure Cognitive Search → LangChain vectorstores with compatible backends.
...
### `AzureChatOpenAI` → `ChatBedrock` migration

**Problem:** `AzureChatOpenAI` requires `azure_deployment` and `model_name`. `ChatBedrock` only requires `model_id`. LangChain's Bedrock client is not a drop-in replacement — initialization parameters differ, and the streaming chunk format differs.

**Fix:** Replace `AzureChatOpenAI(azure_deployment="...", model_name="gpt-4o")` with `ChatBedrock(model_id="anthropic.claude-sonnet-4-6")`. Remove Azure-specific kwargs. Test that all downstream chain components work with the Bedrock response format.

### `with_structured_output` behavior

**Problem:** `AzureChatOpenAI.with_structured_output(schema)` uses OpenAI's native JSON mode or function calling. `ChatBedrock.with_structured_output(schema)` uses Claude's tool use to enforce structure. The output is the same, but error handling differs — Claude may refuse to fill required fields if the prompt doesn't provide enough context, returning a partial object instead of raising an error.
...
**Problem:** `ChatBedrock` streaming works but the chunk format differs from `AzureChatOpenAI`. Code that processes `AIMessageChunk` objects and checks for `tool_call_chunks` may need adjustment.
...
**Problem:** `ChatBedrock` wraps synchronous boto3 calls in threads for async. It's not truly async like `AzureChatOpenAI`'s native async client. Under high concurrency, this can cause thread pool exhaustion.
...
**Fix:** Start fresh threads when switching providers. Don't resume an Azure OpenAI-started thread with a Bedrock model. Use the feature flag approach (separate threads per provider) rather than mid-thread switching.
...
**Problem:** Prompts optimized for Azure OpenAI (GPT-4o) may not perform identically on Claude Sonnet 4.6. Claude tends to be more literal and instruction-following; GPT-4o is more "creative" with ambiguous instructions. System prompts that rely on GPT-4o's implicit behaviors may need adjustment.
...
**Problem:** Azure OpenAI rate limits are per-deployment with clear headers. Bedrock throttling is per-account per-model per-region with different error codes (`ThrottlingException`). Retry logic built for Azure OpenAI's `429` responses may not handle Bedrock's throttling correctly.
...
**Problem:** Azure OpenAI uses tiktoken-compatible tokenizers for GPT models. Bedrock models use different tokenizers (Claude uses its own, Llama uses SentencePiece, etc.). Code that pre-calculates token budgets using tiktoken will be wrong for Bedrock models.
...
**Problem:** Azure OpenAI errors are HTTP errors (400, 401, 429, 500) with Azure-specific error codes. Bedrock errors are AWS SDK exceptions (`ValidationException`, `ThrottlingException`, `ModelNotReadyException`, `AccessDeniedException`). Error handling code built for Azure's error taxonomy won't catch Bedrock failures.

**Fix:** Map error handlers: Azure `401` → Bedrock `AccessDeniedException`, Azure `429` → Bedrock `ThrottlingException`, Azure `400` → Bedrock `ValidationException`. Add handlers for Bedrock-specific errors like `ModelNotReadyException` (model access not yet enabled).
