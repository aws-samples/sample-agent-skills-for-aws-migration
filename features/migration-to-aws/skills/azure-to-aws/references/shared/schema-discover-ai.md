...
- **`discover-iac.md`** when **Azure OpenAI-strong** Terraform evidence is present (see that file's Step 7d), producing a minimal **IaC-inferred** profile so Clarify Category F can run without app code.
...
```json
{
  "metadata": {
    ...
    "profile_source": "application_code|iac_azure_openai|merged",
    "sources_analyzed": {
      "terraform": true,
  ...
  "summary": {
    ...
    "ai_source": "azure_openai|openai|both|other",
      ...
      "evidence": [
        {
          ...
          "line": 12,
          "pattern": "AzureOpenAI(deployment_name=\"gpt-4o\")"
        },
        {
          "source": "terraform",
          "file": "openai.tf",
          "resource": "azurerm_cognitive_account.main",
          "pattern": "Azure Cognitive Services / OpenAI resource"
        }
      ],
  ...
  "infrastructure": [
    {
      "address": "azurerm_cognitive_account.main",
    ...
    {
      "address": "azurerm_user_assigned_identity.openai_identity",
  ...
  "current_costs": {
    ...
    "services_detected": ["Azure OpenAI Service", "Cognitive Services"]
  },

  "detection_signals": [
    {
      "method": "terraform",
      "pattern": "azurerm_cognitive_account resource",
      "confidence": 0.95,
      "evidence": "Found resource 'main' in openai.tf"
    },
    {
      "method": "code",
      "pattern": "openai.AzureOpenAI import",
      "confidence": 0.95,
...
- `metadata.profile_source` — `"application_code"` (full profile from code path), `"iac_azure_openai"` (minimal profile from Terraform Azure OpenAI-strong discovery only), or `"merged"` (code + pre-existing IaC profile combined in `discover-app-code.md`).
...
- `service` — Azure/source service category (NOT `service_type`, `azure_service`)
...
- `ai_source` — Source AI provider in `summary` object: `"azure_openai"`, `"openai"`, `"both"`, or `"other"`
...
- `models[]` — Each distinct AI model/service detected, with evidence and capabilities. **May be empty** for `profile_source: "iac_azure_openai"` when no model IDs can be inferred from Terraform alone; Clarify and Design must not assume non-empty.
