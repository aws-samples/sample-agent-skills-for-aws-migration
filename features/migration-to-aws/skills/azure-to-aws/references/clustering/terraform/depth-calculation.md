...
```
A: azurerm_virtual_network — depends on [] → depth 0
B: azurerm_subnet — depends on [A] → depth 1
C: azurerm_network_security_group — depends on [A] → depth 1
D: azurerm_kubernetes_cluster — depends on [B, C] → depth 2
```
