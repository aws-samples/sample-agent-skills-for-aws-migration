...
IF `$MIGRATION_DIR/azure-resource-inventory.json` exists, add:
...
IF `$MIGRATION_DIR/azure-resource-clusters.json` exists, add:
...
{
  "design_mappings": [
    { "azure_type": "<azure_type>", "aws_service": "<aws_service>", "confidence": "<confidence>" }
  ],
