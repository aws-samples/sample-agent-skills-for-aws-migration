...
Without source configuration (CPU, memory, disk, scaling), billing-only estimates use **Azure-cost-ratio projections** (Step 2 below) rather than precise per-resource AWS pricing lookups. The pricing cache and MCP API are not directly used for per-service calculations — instead, Azure actual spend is scaled by service-type multipliers.
...
- `services[]` — Azure services with AWS mappings and confidence levels
- `metadata.design_source` — must be `"billing_only"`

**From `billing-profile.json`:**

- `services[]` — Azure services with monthly costs and SKUs
- `summary.total_monthly_spend` — Total Azure spend (this is the baseline)

**From `preferences.json`:**

- `design_constraints` — User preferences for optimization approach

## Step 1: Establish Azure Baseline from Billing

Use `billing-profile.json` as the authoritative Azure cost baseline:

```
Azure Monthly Baseline: $[total_monthly_spend]
...
### Reserved-Instance-Aware Baseline Adjustment

If `billing-profile.json` contains `commitments.has_active_reservations == true`:

1. **Use list price as baseline**: The `services[].monthly_cost` values already reflect list price (reservation fee rows are excluded). Use these directly — do not subtract reservation credits from the baseline.
2. **Exclude commitment fees from workload costs**: Reservation fee rows are billing artifacts, not workload costs. They are already excluded from `services[]` by the discover phase.
3. **Note the customer's effective discount**: Record `commitments.effective_discount_percent` for the comparison narrative. The customer currently pays this percentage below list price on Azure.

If `commitments.has_active_reservations == false` or the `commitments` section is absent, proceed with `total_monthly_spend` as-is.
...
Since we lack source configuration, apply percentage-based ranges around the Azure cost:

```
For each service:
  Azure monthly cost: $X
...
| ------------------------------------ | ------------ | ------------------------------------------- |
| Compute (Container Apps, AKS)        | ±30%         | Pricing varies by config and scaling        |
| Database (Azure SQL, Cosmos DB)      | ±35%         | Engine, HA, and sizing drive large variance |
| Storage (Azure Blob Storage)         | ±15%         | Storage pricing is relatively stable        |
| Messaging (Service Bus, Event Hubs)  | ±20%         | Usage-based, somewhat predictable           |
| Networking (Load Balancer, DNS)      | ±20%         | Fixed + usage components                    |
| AI (Azure OpenAI, Cognitive Services)| ±40%         | Model choice drives massive variance        |
...
vs Azure Actual Monthly: $[total_monthly_spend]

Difference:
  Best case:  [low vs Azure] (potential savings of $X/month)
  Expected:   [mid vs Azure] (roughly equivalent cost)
  Worst case: [high vs Azure] (potential increase of $X/month)
```

### Commitment-to-Commitment Comparison (if Reserved Instances detected)

If `billing-profile.json` has `commitments.has_active_reservations == true`, add a commitment context note:

```
Commitment Context:
  Azure current effective rate: [effective_discount_percent]% below list price via Reserved Instances
  Azure list price baseline:   $[cost_basis.total_at_list]/month
  Azure net-of-discounts:      $[cost_basis.total_net_of_discounts]/month
  ...
  Fair comparison: Azure list price vs AWS on-demand (both uncommitted)
  Committed comparison: Azure net-of-reservations vs AWS with 1yr Savings Plan
```

This ensures the customer understands that their current Azure discount has an AWS equivalent, and the comparison is apples-to-apples.
...
**Vendor fees:** If you discuss Azure egress in narrative, describe it only as **vendor/network charges** when grounded in `billing-profile.json` (do not invent dollar amounts). Do not mix human effort into "one-time cost" lists.
...
4. Network topology -- VNet, subnets, peering costs
...
```json
{
  ...
  "unknowns": [
    ...
    },
    {
      ...
      "impact": "medium",
      "resolution": "Describe VNet, subnet, and peering configuration"
    }
  ],
...
- `azure_baseline` matches `billing-profile.json` totals
...
2. Azure baseline from billing data (total monthly spend)
3. AWS projected cost ranges: low / mid / high per service
4. Total projection: best case / expected / worst case vs Azure
