...
## Part 1: Calculate Current Azure Costs

Determine the current Azure monthly infrastructure costs. Use the best available source:

1. **`billing-profile.json` (preferred)** — Use actual billing data as the Azure baseline. Highest confidence (±5%).
2. **`azure-resource-inventory.json` (fallback)** — Estimate costs from discovered resource configurations. Wider range (±20-30%).
3. **`preferences.json` → `azure_monthly_spend`** — User-provided monthly spend from clarification.
4. **Ask the user** — If none of the above are available, ask: "I need your current Azure monthly spend to produce a meaningful cost comparison. What is your approximate Azure monthly infrastructure cost?" Use the user's answer. If the user declines or is unsure, present AWS costs without an Azure comparison and note: "Azure baseline unavailable — AWS costs shown without comparison."

Present the Azure baseline as a total and per-service breakdown, noting which source was used.

### Reserved-Instance-Aware Baseline (when billing data available)

If `billing-profile.json` contains `commitments.has_active_reservations == true`:

1. **Use list price as the Azure baseline**: The `services[].monthly_cost` values already use list price (reservation fee rows excluded by discover phase). Use `cost_basis.total_at_list` as the total Azure baseline for comparison.
2. **Do not include commitment fees in workload costs**: Reservation fee rows are billing artifacts with no AWS equivalent — they are already excluded from `services[]`.
3. **Record the effective discount for Part 3**: Note `commitments.effective_discount_percent` — this is the customer's current Azure discount rate that will be compared against AWS Savings Plan rates.

This ensures the comparison is Azure list price vs. AWS on-demand (both uncommitted baselines), with commitment options presented separately as optimization opportunities.
...
**Secret Manager coverage (mandatory):** If any mapped resource has `azure_type` of `azurerm_key_vault_secret` or `azurerm_key_vault`, ensure an `aws_service` entry for **Secrets Manager** is present in the estimate breakdown. Do not collapse this into a generic "supporting" line item.

**Synapse Analytics / deferred analytics (mandatory):** For any resource where `aws_service` is exactly **`Deferred — specialist engagement`** OR `azure_type` starts with `azurerm_synapse_` OR `azure_type` equals `azurerm_databricks_workspace`:
...
| Storage (S3)               | GB × per-GB rate + request estimates                                                  | `aws_config.storage_gb` or source `azure_config`                              |
...
- Azure current monthly total (at list price)
- AWS Premium / Balanced / Optimized monthly totals
- Difference (savings or increase) per tier vs Azure
- Per-service breakdown for the Balanced tier

### Commitment Context (if Reserved Instances detected)

If `billing-profile.json` has `commitments.has_active_reservations == true`, add a `commitment_context` section to the comparison:

- **Azure effective committed rate**: The customer currently pays `effective_discount_percent`% below list via Reserved Instances
- **AWS equivalent**: 1-year Savings Plans typically save 20-30%; 3-year Savings Plans save 40-60%
- **Fair comparison framing**: Present both an uncommitted comparison (Azure list vs. AWS on-demand) and a committed comparison (Azure net-of-reservations vs. AWS with 1yr SP)
- **Migration timing note**: If the customer has active Reserved Instances, note that reservation fees continue regardless of usage — migrating mid-term means paying both Azure reservation fees and AWS costs until the reservation term expires

Include in `estimation-infra.json` under `cost_comparison`:

```json
"commitment_context": {
  "azure_has_active_reservations": true,
  "azure_effective_discount_percent": 8.2,
  "azure_monthly_at_list": 2450.00,
  "azure_monthly_net_of_discounts": 2280.00,
  "aws_1yr_savings_plan_typical_discount": "20-30%",
  "aws_3yr_savings_plan_typical_discount": "40-60%",
  "note": "Azure baseline uses list price for apples-to-apples comparison. Customer currently saves 8.2% via Reserved Instances. AWS Savings Plans offer comparable or deeper discounts post-migration."
}
```

If `commitments.has_active_reservations == false` or the section is absent, omit `commitment_context` from the output.

---

## Part 4: Azure Data Transfer Egress (Vendor Fees Only)

This section covers **Azure vendor/network charges** for outbound data during migration — not human labor or professional-services costs (those are never presented as dollar estimates by this advisor).
...
**Data transfer** — egress fees from Azure during migration. Azure charges for outbound data transfer; volume depends on database sizes and storage to migrate. Use the billing data to estimate the volume of data that needs to move.
...
**Omit Azure data transfer fee estimates.** Without billing data, there is no grounding for egress projections. Instead, include only this note in the output:
...
```json
{
  ...
  "note": "Data transfer cost estimates require Azure billing data. Re-run discovery with an Azure billing export to see Azure egress fee projections."
}
```

In the user-facing summary, when billing data is missing, state: "Azure data transfer egress estimates require billing data. Provide a billing export and re-run discovery to see vendor egress projections."

---

## Part 5: ROI Analysis

Present the monthly and annual cost difference between Azure baseline and each AWS tier (Premium, Balanced, Optimized). This is the recurring savings (or increase) the customer can expect.
...
- Azure operational efficiency gains: simplified IAM model, consolidated tooling, AWS-native observability
...
**Note:** Azure data transfer egress fees (if estimated in Part 4) are **vendor** one-time charges excluded from recurring ROI calculations — not human migration costs.
...
3. **Stay on Azure (Lowest Cost)** — only if AWS is more expensive and costs are the sole metric
...
- **Stay if:** cost is the only metric and AWS is more expensive, team deeply experienced with Azure, no need for AWS-specific services
