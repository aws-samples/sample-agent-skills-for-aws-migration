...
| API Gateway (Kong, APIM)          | Add Bedrock upstream + SigV4 signing                          | 1-2 config files  |
| Voice Platform (Vapi, Bland.ai)   | Check native Bedrock support, update dashboard                | Dashboard config  |
| Framework (LangChain, Semantic Kernel) | Swap provider import (e.g., `ChatBedrock` for `ChatBedrock` Azure variant) | 1-5 lines of code |
...
| Azure OpenAI SDK (Python)     | boto3 Bedrock Converse API        | `AzureOpenAI.chat.completions.create()` → `bedrock.converse()`                    |
| Azure OpenAI SDK (JS)         | @aws-sdk/client-bedrock-runtime   | `openai.chat.completions.create()` → `client.send(new ConverseCommand())`         |
| Azure OpenAI SDK (Go)         | aws-sdk-go-v2 bedrockruntime      | `azopenai` → `bedrockruntime.Converse()`                                          |
| Azure OpenAI SDK (Java)       | AWS SDK BedrockRuntimeClient      | `OpenAIClient` → `BedrockRuntimeClient.converse()`                                |
| OpenAI SDK                    | boto3 Bedrock Converse API        | `client.chat.completions.create()` → `bedrock.converse()` (if Mantle unavailable) |
| LiteLLM                       | LiteLLM config change             | `model="azure/<deployment>"` → `model="bedrock/anthropic.claude-sonnet-4-6"`      |
| LangChain (Azure variant)     | langchain_aws                     | `AzureChatOpenAI`/`ChatOpenAI` → `ChatBedrock`                                    |
| LlamaIndex                    | llama_index.llms.bedrock_converse | `AzureOpenAI` → `BedrockConverse`                                                 |
...
- `azure_openai` (default) — existing Azure OpenAI or Azure AI provider
...
**Rollback steps:** Set `AI_PROVIDER=azure_openai` (instant), verify source traffic, monitor 1 hour, investigate, re-attempt.
...
**`ai_source` values:** `"azure_openai"`, `"openai"`, `"both"`, `"other"`
...
- [ ] `rollback_plan.default_value` is `"azure_openai"` (or `"openai"` if source is OpenAI-only)
