...
- Header comment: AWS service name, Azure source, confidence level, monthly Azure cost, SKU hints
- `# TODO: Verify all configuration values against actual Azure resource settings`
- At least one `# TODO` per configurable attribute
- Tags: Name, AzureSource, `Confidence = "billing-inferred"`

**Resource generation by Azure service type:**

| Azure Service                                         | AWS Resource Type                   | Key TODO Attributes                                  |
| ----------------------------------------------------- | ----------------------------------- | ---------------------------------------------------- |
| Azure Container Apps, Azure VMs, AKS                  | ECS cluster + task def              | CPU, memory, container image, port, auto-scaling     |
| Azure SQL Database, Azure Database for PostgreSQL/MySQL | RDS instance                      | Engine, version, instance class, storage, networking |
| Azure Cosmos DB, Azure Table Storage                  | DynamoDB / Aurora                   | Capacity, schema, access patterns                    |
| Azure Blob Storage, Azure Files                       | S3 bucket + versioning + encryption | Bucket count, storage classes, lifecycle policies    |
| Azure Application Gateway, Azure Load Balancer        | VPC + ALB                           | CIDR, subnets, security groups, listeners            |
| Azure DNS                                             | Route 53                            | Zone names, record types                             |
| Azure NAT Gateway                                     | NAT Gateway                         | Subnet associations                                  |
...
- [ ] Tags use `AzureSource` (not `GCPSource`) to identify the migration origin
...
- [ ] skeleton.tf resource header comments reference Azure service names and SKUs (not GCP service names)
...
WARNING: These are SKELETON configurations generated from Azure billing data only.
