...
**Sparse / IaC-only profile:** If `ai-workload-profile.json` has empty `models[]` or `metadata.profile_source` is `iac_azure`, use `aws-design-ai.json` for Bedrock targets and example prompts; do **not** fail solely because discovery did not list model IDs.
...
**Determine AI source** from `aws-design-ai.json` → `ai_architecture.ai_source`. Supported values for Azure-to-AWS: `"azure_openai"`, `"openai"`, `"both"`, `"other"`.
...
- Read `AI_PROVIDER` env var to select provider: `azure_openai` (current), `bedrock` (target), `shadow` (both — return source response, log Bedrock response)
...
- **Source provider class**: Use SDK imports from `ai-workload-profile.json` → `integration.sdk_imports`. Use model IDs from `ai-workload-profile.json` → `models[].model_id`. For Azure OpenAI source, use `openai.AzureOpenAI()` with `azure_endpoint`, `api_key`, and `api_version` from the profile.
...
**Python example (Azure OpenAI source):**
...
AI_PROVIDER = os.environ.get("AI_PROVIDER", "azure_openai")
...
class AzureOpenAIProvider:
    def __init__(self):
        ...
        self.deployment = os.environ.get("AZURE_OPENAI_DEPLOYMENT", "<deployment-name>")
...
class BedrockProvider:
    def __init__(self):
        self.client = boto3.client("bedrock-runtime", region_name=os.environ.get("AWS_REGION", "<target-region>"))
        self.model_id = os.environ.get("BEDROCK_MODEL_ID", "<aws-model-id>")
...
def get_provider():
    ...
    elif AI_PROVIDER == "shadow":
        return ShadowProvider(AzureOpenAIProvider(), BedrockProvider())
    else:
        return AzureOpenAIProvider()
```

For JS: use `@aws-sdk/client-bedrock-runtime` + `openai` (AzureOpenAI client). For Go: use `github.com/aws/aws-sdk-go-v2/service/bedrockruntime` + the Azure OpenAI REST client.
...
- Run each prompt against both the source provider (Azure OpenAI) and Bedrock
...
- Step 3 — Print required environment variables: `AWS_REGION`, `AI_PROVIDER=bedrock`, model IDs; also print the Azure OpenAI env vars that should be retired: `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_DEPLOYMENT`
...
- Remove any `AZURE_OPENAI_*` env var references from the config
...
- LangChain with Azure OpenAI source: `langchain_openai.AzureChatOpenAI` → `langchain_aws.ChatBedrock`
- LangChain with OpenAI source: `langchain_openai.ChatOpenAI` → `langchain_aws.ChatBedrock`
- Semantic Kernel: `semantic_kernel.connectors.ai.open_ai.AzureChatCompletion` → LangChain + Bedrock adapter (`langchain_aws.ChatBedrock` wrapped in a Semantic Kernel plugin)
- LlamaIndex: `llama_index.llms.azure_openai` → `llama_index.llms.bedrock_converse`
- Include pip install note for the AWS package (e.g., `pip install langchain-aws boto3`)
- Remove Azure OpenAI dependency from requirements: drop `openai`, `azure-ai-openai`, `azure-identity`
...
# Generated by Azure-to-AWS migration plugin
...
```bash
#!/bin/bash
# Incremental Migration: Azure OpenAI → Bedrock via AgentCore Harness
...
# Phase 0: Store Azure OpenAI API key in AgentCore Identity
echo "--- Phase 0: Configure source provider credentials ---"
echo "Run once:"
echo "  agentcore add credential --type api-key --name azure-openai-key --api-key \$AZURE_OPENAI_API_KEY"
echo "  agentcore deploy"
echo ""

# Phase 1: Invoke with Azure OpenAI model on AgentCore infrastructure
echo "--- Phase 1: Azure OpenAI on AgentCore ---"
echo "agentcore invoke --harness $HARNESS_NAME \\"
echo "  --model-provider $SOURCE_PROVIDER \\"
echo "  --model-id $SOURCE_MODEL \\"
echo "  --api-key-arn arn:aws:bedrock-agentcore:{region}:{account}:token-vault/default/apikeycredentialprovider/azure-openai-key \\"
...
# Phase 4: Remove Azure OpenAI credentials
echo "--- Phase 4: Clean up Azure OpenAI credentials ---"
...
```python
"""
Strands Agents Migration — Generated by Azure-to-AWS migration plugin
...
```python
"""
Bridge: OpenAI Agents SDK (Azure OpenAI backend) → Bedrock (via OpenAI-compatible endpoint)
...
# The model calls route through Bedrock instead of Azure OpenAI.
...
print(f"Bridge configured: {BRIDGE_CONFIG['source_model']} ({BRIDGE_CONFIG['source_provider']}) → {BRIDGE_CONFIG['target_model']}")
...
- [ ] Feature flag (`AI_PROVIDER` env var) controls provider selection in adapter; default value is `azure_openai` (not `vertex_ai`)
- [ ] Source provider class uses Azure OpenAI SDK (`openai.AzureOpenAI`) for `ai_source = "azure_openai"`, or standard OpenAI SDK for `ai_source = "openai"`
...
- [ ] No `azure-ai-openai`, `azure-identity`, or other Azure SDK packages added to requirements when they should be removed
- [ ] Framework gateway config replaces `AzureChatOpenAI` / `AzureChatCompletion` with `ChatBedrock` (not Vertex AI equivalents)
...
AI source: [ai_source value from aws-design-ai.json]
