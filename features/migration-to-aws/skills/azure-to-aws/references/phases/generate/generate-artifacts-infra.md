...
- `azure-resource-clusters.json` (REQUIRED) — Cluster dependency graph for ordering
...
**Synapse / specialist-deferred:** If `aws_service` is **`Deferred — specialist engagement`**, **do not** generate Terraform for that resource (no Glue, Athena, Redshift, or EMR modules from the plugin). Optionally add **`terraform/README-SYNAPSE-DEFERRED.md`** with a short checklist: engage **AWS account team** and/or **data analytics migration partner** before implementing analytics infrastructure.
...
3. **Which scenario this Terraform matches** — **Balanced** (primary comparison to Azure; default migration posture in the advisor model). Premium and Optimized are **not** auto-generated as alternate roots.
...
**Per-cluster variables:** Extract configurable values from `aws_config` in `aws-design.json`. Infer types (`string`, `number`, `bool`, `list(string)`, `map(string)`). Use `aws_config` values as defaults. Deduplicate shared variables. Add Azure source as comment (e.g., `# Azure source: Standard_D2s_v3`).
...
- A single Azure resource may map to multiple AWS resources (1:Many expansion)
- Use `azure_config` values from `aws-design.json` to populate resource attributes
...
| `source_cloud`                        | string         | Always **`"azure"`** — identifies the migration source platform                                                                                                |
...
- [ ] `migration_summary` output includes `aligned_with_estimate_tier`, `cost_scenarios_modeled_in_terraform`, and `source_cloud = "azure"`
...
- [ ] Variables file uses Azure source comments (e.g., `# Azure source: ...`), not GCP source comments.
