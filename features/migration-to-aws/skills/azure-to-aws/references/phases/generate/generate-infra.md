...
- `azure-resource-clusters.json` (REQUIRED) — Cluster dependency graph from Phase 1
...
> **Before building the timeline, load `references/shared/migration-complexity.md` and classify the migration tier using inputs from `aws-design.json`, `azure-resource-clusters.json`, `estimation-infra.json`, and `preferences.json`.**

Build a timeline using the **Infrastructure Path** ranges for the determined tier. Also consider:

- **Cluster count** from `azure-resource-clusters.json` — more clusters = longer infrastructure phase
...
- Keep Azure resources running as hot standby

#### Stage 4: Validation + Cleanup (Weeks 5-6)

- Monitor AWS performance for 1 week
- Compare actual costs against projections
- Begin Azure teardown planning (execute teardown after 2-week stability period)
...
- Establish Azure-to-AWS connectivity (VPN or Direct Connect for data migration)
- Set up CI/CD pipeline for Terraform deployments
...
- Validate application connectivity and performance
- Test data pipeline from Azure to AWS (database replication, storage sync)
...
- **Databases**: Set up continuous replication (Azure SQL/Azure Database for PostgreSQL to RDS/Aurora)
  - Initial full snapshot transfer
  - Enable ongoing replication (AWS DMS or native replication)
  - Validate data integrity (row counts, checksums)
- **Object storage**: Sync Azure Blob Storage containers to S3
  - Use AWS DataSync or AzCopy + `aws s3 sync` for bulk transfer
  - Enable ongoing sync for new objects
- **Secrets**: Migrate secrets from Azure Key Vault to AWS Secrets Manager
...
Extended monitoring before Azure teardown.
...
- If **stateful services** (Azure Cache for Redis, Elasticsearch): Add "State migration data loss" (low/critical)
- If **multiple regions** in Azure: Add "Cross-region dependency during migration" (medium/medium)
- If **AI workloads** coexist: Add "Model performance drift on Bedrock" (medium/high)
- If **Synapse Analytics** resources exist (`azurerm_synapse_*` or `aws_service` is **`Deferred — specialist engagement`** in `aws-design.json`): Add "Synapse Analytics migration complexity" (high/high) with mitigation: "**No automated AWS analytics target** — engage **AWS account team** and/or **data analytics migration partner** before architecture or cost commitments; plugin does not prescribe Athena/Redshift/Glue."
...
| Latency      | Within 10% of Azure baseline                     | P50, P95, P99 response times    |
...
- Performance regression > 20% vs Azure baseline (P95 latency)
...
| PoC (3-4)            | Tear down PoC cluster; Azure unaffected              | < 1 hour     |
| Infrastructure (5-7) | Tear down AWS clusters; Azure still primary          | < 2 hours    |
| Data Migration (8-9) | Stop replication; Azure data is authoritative        | < 30 minutes |
| Cutover (10-11)      | Reverse DNS to Azure; resume Azure traffic           | < 1 hour     |
| Post-Cutover (12+)   | Manual restore from backup; Azure resources archived | 2-4 hours    |

### Rollback Procedure (Pre-DNS Cutover)

1. Pause dual-write replication
2. Reverse DNS records (AWS to Azure endpoints)
3. Verify Azure services receiving traffic
4. Shut down AWS workloads (keep for 1 week as standby)
5. Monitor Azure for 24 hours
6. Post-mortem: document what triggered rollback

### Rollback Procedure (Post-DNS Cutover)

1. If within 48 hours: Reverse DNS, resume Azure from replicated state
2. If beyond 48 hours: Restore Azure from backup (2-4 hour RTO)
3. Validate data integrity after restore
4. Resume Azure operations
...
- Compare latency/throughput to Azure baseline daily
- Review AWS costs daily vs projections
- Maintain Azure hot standby

**Week 2 (Days 8-14)**: Stabilization

- Reduce monitoring frequency to twice daily
- Begin decommissioning Azure hot standby (read-only mode)
...
- Archive Azure resources and begin teardown
...
```json
{
  ...
  "migration_plan": {
    ...
    "services": [
      {
        ...
        "aws_service": "Fargate",
        "migration_week": 5,
        "cluster_id": "compute_containerapps_eastus_001",
        "estimated_effort_hours": 40,
        "data_migration_required": false, // derive from resource detection flags (has_databases, has_storage)
        "human_expertise_required": false // propagate from aws-design.json; true for Synapse Analytics mappings
      }
    ],
    "critical_path": [
      "VPC setup (Week 1)",
    ...
    "dependencies": [
      {
        "from_cluster": "networking_vnet_eastus_001",
        "to_cluster": "compute_containerapps_eastus_001",
        "type": "must_precede"
      }
    ]
  },
    ...
    "per_service": [
      {
        ...
        "latency_target": "within 10% of Azure baseline",
...
- `migration_plan.dependencies` reflect `azure-resource-clusters.json` creation_order
...
- All cluster IDs reference valid clusters from `azure-resource-clusters.json`
