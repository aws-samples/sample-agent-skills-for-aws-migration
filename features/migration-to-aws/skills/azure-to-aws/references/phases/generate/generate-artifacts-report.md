...
| --------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------ |
| Azure services detected                 | `aws-design.json` clusters[].resources[]                     | `aws-design-billing.json` services[]             |
...
| Current Azure monthly cost              | `estimation-infra.json` current_costs.azure_monthly          | `estimation-billing.json`                        |
...
- Count of Azure services detected
- List each Azure service with its type (e.g., "Azure App Service (compute)", "Azure SQL Database (database)")
...
- Side-by-side display: Current Azure Monthly vs Projected AWS Monthly (**Balanced** tier — the default scenario for comparing to Azure)
- Percent change (savings or increase)
- **How to read cost tiers (callout box — required when infra estimation with three tiers exists):** The three AWS monthly figures are **pricing scenarios** for the **same** mapped architecture (same services in `aws-design.json`), not three different generated Terraform stacks. **Order = highest → middle → lowest** monthly estimate in this model. Use **Balanced** as the **primary** row vs Azure; **Premium** and **Optimized** are **bounds** (higher HA / newer skew vs cost-optimization skew). When `terraform/` is present, it implements **one** infrastructure baseline aligned with the **Balanced** cost scenario (see `terraform/README.md` and `migration_summary` output).
...
**Three-tier comparison table** with columns: **Tier** (name + subtitle as in Section 3), Monthly Cost, vs Azure Monthly, Annual Difference.
...
- Model mappings (Azure OpenAI / OpenAI model to AWS Bedrock model)
- AI cost estimates
...
<head>
  ...
  <title>Azure to AWS Migration Assessment</title>
  <style>
...
<body>
  <div class="report">
    ...
    <footer>
      Generated by Azure to AWS Migration Advisor
    </footer>
  </div>
</body>
</html>
...
6. **Azure branding**: All source-cloud references use "Azure" — no "GCP" or "Google Cloud" text anywhere in the report.
...
6. **Azure terminology**: HTML title is "Azure to AWS Migration Assessment"; all source-cloud labels say "Azure"; footer says "Azure to AWS Migration Advisor"
