...
- If **`google_bigquery_*`** or **`Deferred — specialist engagement`** appears in `aws-design.json`: Add "BigQuery / analytics architecture undefined" (high/high) with mitigation: "**No automated AWS analytics target** — engage **AWS account team** and/or **data analytics migration partner** before warehouse or lake design; plugin does not prescribe Athena/Redshift/Glue."
