...
| Security   | Least-privilege IAM (specific ARNs, never wildcards); per-service roles for Fargate/Lambda; Secrets Manager resources with no plaintext defaults; for every `aws_secretsmanager_secret` resource emit a companion `aws_secretsmanager_secret_rotation` block with `automatically_after_days = 30` and a TODO comment for the rotation Lambda ARN — rotation is a baseline requirement for all compliance frameworks |
