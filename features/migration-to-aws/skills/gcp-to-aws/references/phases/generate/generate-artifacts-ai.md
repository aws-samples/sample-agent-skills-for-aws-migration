...
- `ai-migration/setup_bedrock.sh` and `ai-migration/test_comparison.py` must exist.
- If direct SDK path is active, a `provider_adapter` file exists (`.py`, `.js`, or `.go`).
- If gateway path is active, a `gateway_config` file exists (`.yaml`, `.py`, or `.json`).
- If evaluation path is active, both `eval-prompts.jsonl` and `run-evaluation.sh` exist.

If this gate fails: STOP and output: "generate-artifacts-ai did not produce required AI artifacts for the selected path."
