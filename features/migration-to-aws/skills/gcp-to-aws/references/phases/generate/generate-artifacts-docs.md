...
**Human Expertise Advisory (BigQuery / deferred analytics)** — If any service in the plan has `human_expertise_required: true` for BigQuery or `aws_service` is **`Deferred — specialist engagement`**, include a prominent callout in Section 4 next to that service:
