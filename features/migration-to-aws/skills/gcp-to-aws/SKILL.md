...
- **BigQuery / `google_bigquery_*`**: The skill **does not** recommend a specific AWS analytics or warehouse service. Design output uses **`Deferred — specialist engagement`**; direct the user to their **AWS account team** and/or a **data analytics migration partner** (see `references/phases/design/design-infra.md` BigQuery specialist gate).
